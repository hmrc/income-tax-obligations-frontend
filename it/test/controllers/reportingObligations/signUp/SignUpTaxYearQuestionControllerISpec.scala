...
import models.audit.SignUpAuditModel
    ...
    s"a user is a $mtdUserRole" that {
      ...
      "is authenticated with valid enrolment" should {
        ...
        "redirect back to the reporting obligations page if the user has no annual CY or CY+1" in {
          ...
          val reportingObligationsLink = mtdUserRole match {
            case MTDIndividual => appConfig.baseUrl + "/reporting-frequency"
            case _ => appConfig.agentBaseUrl + "/reporting-frequency"
          }
      ...
      "submit the answer to the sign up tax year question - SingleYear" in {
        ...
        whenReady(buildPOSTMTDPostClient(s"$path?taxYear=$currentYear", additionalCookies, Map("sign-up-tax-year-question" -> Seq("Yes")))) { result => {
          ...
          result should have(
            httpStatus(SEE_OTHER),
            redirectURI(s"${appConfig.baseUrl}$path/completed")
          )
        }
        }
      }
        ...
        "redirect to the cannot go back page" in {
          ...
          val redirectUrl = {
            if(mtdUserRole != MTDIndividual)
              controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = true, isSignUpJourney = Some(true)).url
            else
              controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = false, isSignUpJourney = Some(true)).url
          }
