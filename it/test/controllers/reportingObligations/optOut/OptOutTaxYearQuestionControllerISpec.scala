...
import helpers.servicemocks.{AuditStub, ITSAStatusDetailsStub}
...
import models.audit.OptOutNewAuditModel
...
import models.{GroupedObligationsModel, ObligationsModel, SingleObligationModel, StatusFulfilled, StatusOpen, UIJourneySessionData}
      ...
      "has already completed the journey (according to session data)" should {
        "redirect to the cannot go back page" in {
          ...
          val redirectUrl: String =
            if (mtdUserRole != MTDIndividual) controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = true, isSignUpJourney = Some(false)).url
            else controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = false, isSignUpJourney = Some(false)).url
          val result = buildGETMTDClient(s"$path?taxYear=$currentYear", additionalCookies).futureValue
