...
class SignUpTaxYearQuestionViewSpec extends TestSupport {
  ...
  class Setup(viewModel: SignUpTaxYearQuestionViewModel, withError: Boolean = false) {
    ...
    val pageDocument = org.jsoup.Jsoup.parse(contentAsString(signUpTaxYearQuestion(viewModel, form, controllers.reportingObligations.signUp.routes.SignUpTaxYearQuestionController.submit(isAgent = true))))
  }
