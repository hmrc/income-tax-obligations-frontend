...
class SignUpTaxYearQuestionControllerSpec extends MockAuthActions with MockSignUpService {
  ...
  private def reportingObligationsLink(isAgent: Boolean): String = {
    if (isAgent) {
      appConfig.agentBaseUrl + "/reporting-frequency"
    } else {
      appConfig.baseUrl + "/reporting-frequency"
    }
  }
  ...
  private def confirmPageLink(isAgent: Boolean): String = {
    if (isAgent) {
      appConfig.agentBaseUrl + "/sign-up/completed"
    } else {
      appConfig.baseUrl + "/sign-up/completed"
    }
  }
    ...
    s"show(isAgent = $isAgent)" when {
      ...
      s"the user is authenticated as a $mtdRole" should {
        ...
        "redirect the user to the home page when the sign up and opt in opt out feature switches are disabled" in {
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result).get shouldBe homeLink(isAgent)
        }
    ...
    s"submit(isAgent = $isAgent)" when {
      ...
      s"the user is authenticated as a $mtdRole" should {
        ...
        "redirect the user when they select 'Yes' - to the completion page" in {
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result).get shouldBe confirmPageLink(isAgent)
        }
        ...
        "redirect to the reporting obligations page when the user selects 'No'" in {
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result).get shouldBe reportingObligationsLink(isAgent)
        }
