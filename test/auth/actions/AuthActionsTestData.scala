...
import auth.{AgentClientDetails, AuthUserDetails, AuthorisedAgentWithClientDetailsRequest, AuthorisedAndEnrolledRequest, AuthorisedUserRequest}
...
object AuthActionsTestData extends TestSupport {

  lazy val mtdEnrolment = Enrolment("HMRC-MTD-IT", Seq(EnrolmentIdentifier("MTDITID", testMtditid)), "Activated", None)
  lazy val agentEnrolment = Enrolment("HMRC-AS-AGENT", Seq(EnrolmentIdentifier("AgentReferenceNumber", testArn)), "Activated", None)
  lazy val ninoEnrolment = Enrolment("HMRC-NI", Seq(EnrolmentIdentifier("NINO", testNino)), "Activated", None)
  lazy val saEnrolment = Enrolment("IR-SA", Seq(EnrolmentIdentifier("UTR", testSaUtr)), "Activated", None)
  lazy val testCredentials = Credentials(testCredId, "bar")
  lazy val defaultIncomeSourcesData = IncomeSourceDetailsModel(testNino, testSaUtr, Some("2012"), Nil, Nil)
  lazy val invalidIncomeSourceData = IncomeSourceDetailsError(500, "mongo error")

  lazy val acceptedConfidenceLevel: ConfidenceLevel = ConfidenceLevel.L250
  lazy val notAcceptedConfidenceLevel: ConfidenceLevel = ConfidenceLevel.L50
  ...
  def getAuthUserDetails(affinityGroup: Option[AffinityGroup],
                         ...
                         hasUserName: Boolean,
                         hasCredentials: Boolean = true): AuthUserDetails = 
    AuthUserDetails(
      enrolments,
      affinityGroup,
